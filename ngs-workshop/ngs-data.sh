...
if [ $(stat -c %U "$top_dir/$working_dir") != "$sudoer_user" ]; then
  echo "Making working directory owned by $sudoer_user"
  chown "$sudoer_user" "$top_dir/$working_dir"
else
  echo "Working directory already owned by $sudoer_user"
fi

if [[ ! -e "/home/${sudoer_user}/Desktop" ]]; then
  mkdir --mode=755 /home/${sudoer_user}/Desktop
  chown -R "$sudoer_user:$sudoer_user" /home/${sudoer_user}/Desktop
fi
...
if [ $(stat -c %U "$top_dir/$working_dir/$module_dir") != "$sudoer_user" ]; then
  echo "  Making module's working directory ($top_dir/$working_dir/$module_dir) owned by $sudoer_user"
  chown -R "$sudoer_user:$sudoer_user" "$top_dir/$working_dir/$module_dir"
fi
####################
